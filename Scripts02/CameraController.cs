...
public class CameraController : MonoBehaviour {
	...
	void CheckUnitSelected(string unitName){
		if (unitName != null) {
	...
	void Update () {
		...
		if (Input.GetButtonDown ("UnitTurnLock")) {
			camButtonDown = true;
		}

		if (Input.GetButtonUp ("UnitTurnLock")) {
			camButtonDown = false;
		}
	...
	void LateUpdate(){
		...
		if (camButtonDown == true) {
						...
						position = rotation * new Vector3 (0.0f, 0.0f, (-camDistance) - 4.0f) + cameraTarget.position;
